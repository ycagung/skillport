<script lang="ts">
	import { onboardingExperiencesSchema } from '$lib/schema';
	import SuperDebug, {
		superForm,
		type Infer,
		type SuperValidated
	...
	let {
		...
		data: SuperValidated<Infer<typeof onboardingExperiencesSchema>>;
		embla: CarouselAPI | undefined;
		completed: boolean;
	} = $props();

	const form = superForm(data, {
		validators: zod4Client(onboardingExperiencesSchema),
		dataType: 'json',
		invalidateAll: false,
